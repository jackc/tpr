...
func CountPasswordReset(ctx context.Context, db Queryer) (int64, error) {
  var n int64
  err := prepareQueryRow(ctx, db, "pgxdataCountPasswordReset", countPasswordResetSQL).Scan(&n)
  return n, err
}
...
func SelectAllPasswordReset(ctx context.Context, db Queryer) ([]PasswordReset, error) {
  ...
  dbRows, err := prepareQuery(ctx, db, "pgxdataSelectAllPasswordReset", SelectAllPasswordResetSQL)
  if err != nil {
    return nil, err
  }
...
func SelectPasswordResetByPK(
  ...
  var row PasswordReset
  err := prepareQueryRow(ctx, db, "pgxdataSelectPasswordResetByPK", selectPasswordResetByPKSQL, token).Scan(
&row.Token,
    &row.Email,
    &row.RequestIP,
    &row.RequestTime,
...
returning "token"
  ...
  return prepareQueryRow(ctx, db, psName, sql, args...).Scan(&row.Token)
}
...
func DeletePasswordReset(ctx context.Context, db Queryer,
  ...
  commandTag, err := prepareExec(ctx, db, "pgxdataDeletePasswordReset", sql, args...)
  if err != nil {
    return err
  }
  if commandTag.RowsAffected() != 1 {
