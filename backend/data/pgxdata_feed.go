...
func CountFeed(ctx context.Context, db Queryer) (int64, error) {
  var n int64
  err := prepareQueryRow(ctx, db, "pgxdataCountFeed", countFeedSQL).Scan(&n)
  return n, err
}
...
func SelectAllFeed(ctx context.Context, db Queryer) ([]Feed, error) {
  ...
  dbRows, err := prepareQuery(ctx, db, "pgxdataSelectAllFeed", SelectAllFeedSQL)
  if err != nil {
    return nil, err
  }
...
func SelectFeedByPK(
  ...
  var row Feed
  err := prepareQueryRow(ctx, db, "pgxdataSelectFeedByPK", selectFeedByPKSQL, id).Scan(
&row.ID,
    &row.Name,
    &row.URL,
    &row.LastFetchTime,
...
func DeleteFeed(ctx context.Context, db Queryer,
  ...
  commandTag, err := prepareExec(ctx, db, "pgxdataDeleteFeed", sql, args...)
  if err != nil {
    return err
  }
  if commandTag.RowsAffected() != 1 {
